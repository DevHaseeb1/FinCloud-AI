...
function MiniChart() {
  const bars = [20, 45, 30, 55, 40, 60, 50, 70, 55, 65, 75, 60];
  ...
  return (
    <div className="flex items-end gap-1.5 h-24" aria-hidden>
      {bars.map((h, i) => (
        <div
          key={i}
          className="w-3 rounded-t-sm bg-gradient-to-t from-cyan/60 to-cyan/30"
    ...
    <div className="flex min-h-[100dvh] w-full">
      ...
      <div className="relative hidden w-1/2 flex-col items-center justify-center bg-[#0A0E1A] p-12 lg:flex">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <p className="mt-2 text-sm text-white/60">Cloud cost intelligence</p>
        </div>
        <div
          className="relative z-10 mt-12 w-full max-w-xs"
          style={{
            opacity: reduced ? 1 : 0,
            animation: reduced ? "none" : "fade-up 400ms var(--ease-out-expo) 100ms forwards",
          }}
        >
          <MiniChart />
        </div>
